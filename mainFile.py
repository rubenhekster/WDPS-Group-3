...
def ner((x, text)):
    ...
    sentences = [nltk.word_tokenize(sent.decode('utf-8')) for sent in sentences]
    sentences = [nltk.pos_tag(sent.decode('utf-8')) for sent in sentences]
    chunks = [nltk.ne_chunk(sent.decode('utf-8')) for sent in sentences]
    return (x, chunks)
